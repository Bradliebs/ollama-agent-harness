...
export interface TeammateSettings {
  ...
  nextRunAt: string;
  /**
   * Optional custom briefing instruction. When set (non-empty), the brief is
   * generated by running this prompt through the model + web search instead of
   * the default activity snapshot. Empty/absent → default daily activity brief.
   */
  briefingPrompt?: string;
  /** Soft+hard word cap for a custom briefing. Default 150 when unset. */
  briefingMaxWords?: number;
  /** Optional path to a local .ics file used to add an agenda to the briefing. */
  calendarPath?: string;
}
...
export function sanitizeTeammateSettings(value: unknown): TeammateSettings {
  ...
  const channels = Array.isArray(source.channels)
    ...
    : defaults.channels;
  const briefingPrompt = typeof source.briefingPrompt === 'string' ? source.briefingPrompt.slice(0, 2000) : defaults.briefingPrompt;
  const briefingMaxWords = typeof source.briefingMaxWords === 'number' && Number.isFinite(source.briefingMaxWords)
    ? Math.min(1000, Math.max(20, Math.round(source.briefingMaxWords)))
    : defaults.briefingMaxWords;
  const calendarPath = typeof source.calendarPath === 'string' ? source.calendarPath.slice(0, 1000).trim() : defaults.calendarPath;
  return {
    enabled,
    scheduleTime,
