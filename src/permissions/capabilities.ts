...
export const CAPABILITY_POLICIES: CapabilityPolicy[] = [
  {
    ...
    category: 'computer',
    posture: 'design-only',
    summary: 'Desktop control is not implemented; future support needs a user-visible automation bridge and emergency stop.',
    readiness: 'No desktop-control connector exists yet.',
    existingCoverage: ['kill switch', 'permission prompts'],
    missingConnectors: ['desktop automation adapter', 'screen state capture', 'input replay sandbox'],
  ...
  },
  {
    ...
    category: 'browser',
    posture: 'design-only',
    summary: 'Browser profile data is not accessed directly; future access needs scoped profile grants and redaction.',
    readiness: 'Harness can use web tools but does not read browser profiles.',
    existingCoverage: ['web_fetch', 'web_search', 'web_read'],
    missingConnectors: ['browser profile connector', 'cookie/session vault', 'profile scope picker'],
