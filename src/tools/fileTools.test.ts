...
describe('file tools bounds and path safety', () => {
  ...
  describe('agent-outputs redirect', () => {
    ...
    it('does NOT redirect new relative subpaths even when Agent Files override is set', async () => {
      // Subdirectory paths like "src/foo.ts" or "fine-tuning/python/trainer.py"
      // are deliberate writes to a specific repo subtree; the Agent Files
      // override must NOT silently capture them. Only bare filenames and
      // explicit "agent-outputs/" opt-ins should be redirected.
      const subPath = path.join('reports', `keep-${Date.now()}.txt`);
      const result = await FileWriteTool.execute({ path: subPath, content: 'subdir' });
      try {
        // The write should land in the project root (or be rejected by the
        // path-confinement layer). The critical assertion: it MUST NOT have
        // landed inside the override directory.
        expect(result.output).not.toContain('redirected from bare filename');
        const wronglyRedirected = path.resolve(overrideDir, subPath);
        await expect(fs.access(wronglyRedirected)).rejects.toThrow();
      } finally {
        await fs.rm(path.join(process.cwd(), 'reports'), { recursive: true, force: true });
      }
    });

    it('honours an explicit agent-outputs/ prefix as opt-in for the override dir', async () => {
      // The agent (or user) can opt INTO the override by naming the
      // agent-outputs/ directory explicitly. The path under it is preserved
      // so multi-file scratch projects still group together.
      const subPath = path.join('agent-outputs', 'reports', `opted-${Date.now()}.txt`);
      const result = await FileWriteTool.execute({ path: subPath, content: 'opted in' });
      try {
        expect(result.success).toBe(true);
        expect(result.output).toContain('redirected');
        const expected = path.resolve(overrideDir, 'reports', path.basename(subPath));
        expect(result.output).toContain(expected);
        const written = await fs.readFile(expected, 'utf-8');
        expect(written).toBe('opted in');
      } finally {
  ...
  describe('external path E2E matrix', () => {
    ...
    it('does NOT redirect nested relative paths to the external directory', async () => {
      // Mirror the in-project subpath test: a subdir write is deliberate
      // and must not be silently relocated to the external Agent Files dir.
      ...
      try {
        expect(result.output).not.toContain('redirected from bare filename');
        const wronglyRedirected = path.join(externalDir, sub);
        await expect(fs.access(wronglyRedirected)).rejects.toThrow();
      } finally {
        await fs.rm(path.join(process.cwd(), 'analysis'), { recursive: true, force: true });
      }
    });
