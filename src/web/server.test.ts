...
describe('web server API validation', () => {
  ...
  it('persists disabledTools and killSwitch state via /api/settings', async () => {
    ...
    try {
      ...
      // Cleanup so subsequent tests start in a known state, even if assertions above failed.
  ...
  it('sets timed autonomy and returns expiry and previous mode', async () => {
    ...
    try {
      ...
      // Cleanup
      await request('/api/permissions/timed-autonomy', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
  ...
  it('returns parsed merge proposals and applies them via /api/curator/proposals/apply', async () => {
    ...
    // need to reach an LLM during tests. Use a random suffix so a running
    // dev-server watcher (or parallel jest worker) cannot collide on the
    // same .harness/skills/ subdirectories.
    const suffix = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    const nameA = `apply-test-a-${suffix}`;
    const nameB = `apply-test-b-${suffix}`;
    const nameUmbrella = `apply-test-umbrella-${suffix}`;
    const skillA = path.join(process.cwd(), '.harness', 'skills', nameA);
    const skillB = path.join(process.cwd(), '.harness', 'skills', nameB);
    ...
    await fs.writeFile(proposalsFile, `### Cluster: ${nameUmbrella}\n- merge: ${nameA}, ${nameB}\n- rationale: testing\n`, 'utf-8');
    try {
      ...
      const target = body.proposals.find((p) => p.umbrellaName === nameUmbrella);
      expect(target).toBeDefined();
      expect(target?.mergeSkills).toEqual([nameA, nameB]);
      ...
      expect(result.result.archived).toEqual([nameA, nameB]);
      const umbrella = await fs.readFile(path.join(process.cwd(), '.harness', 'skills', nameUmbrella, 'SKILL.md'), 'utf-8');
      expect(umbrella).toContain(nameA);
      expect(umbrella).toContain(nameB);

      const dismiss = await request('/api/curator/proposals', { method: 'DELETE' });
      expect(dismiss.status).toBe(200);
    } finally {
      // force:true + best-effort retry: a file watcher in another process
      // may briefly hold a handle, so we tolerate transient EBUSY here.
      ...
      await safeRm(path.join(process.cwd(), '.harness', 'skills', nameUmbrella));
      await safeRm(path.join(process.cwd(), '.harness', 'skills', '_archive', nameA));
      await safeRm(path.join(process.cwd(), '.harness', 'skills', '_archive', nameB));
      await safeRm(skillA);
      await safeRm(skillB);
  ...
  describe('static asset security headers', () => {
    it('serves the dashboard with clickjacking, sniffing, and referrer protection headers', async () => {
    ...
    it('embeds a Content-Security-Policy meta tag restricting resource origins', async () => {
      const response = await request('/');
      expect(response.status).toBe(200);
